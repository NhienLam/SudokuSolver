...
public class SudokuSolverGrader {
	...
	{
		String err;
		Class<?> className = getClass(PACKAGE + ".SudokuGrid");
		if (className == null)
		{
			sop("No Grid Class");
		}
		...
		SudokuGrid g1 = TestGrid.getPuzzle1();
		SudokuGrid g2 = TestGrid.getPuzzle1();
		...
		SudokuGrid g3 = new SudokuGrid(puzzleTestEqual);
		if(!g1.equals(g2))
		{
		...
		if(new SudokuGrid(puzzleTestisLegalRow).isLegal())
		{
			deduct(Category.Grid, "isLegal() returns True for an incorrect Grid", 2);
		}
		if(new SudokuGrid(puzzleTestisLegalCol).isLegal())
		{
			deduct(Category.Grid, "isLegal() returns True for an incorrect Grid", 2);
		}
		//check for isFull
		...
		if(!new SudokuGrid(testisFull).isFull())
		{
	...
	public void gradeSolver()
	{
		...
		SudokuGrid g2 = new SudokuGrid(puzzleTestisLegalRow);
		SudokuGrid g3 =new SudokuGrid(puzzleTestisLegalCol);
		SudokuGrid g4 = new SudokuGrid(puzzleTestisLegalGrid);
		SudokuGrid g5 = TestGrid.getSolution1();
		try
		{
		...
		SudokuGrid g = TestGrid.getPuzzle1();
		Solver solver = new Solver(g);
		solver.solve();
		SudokuGrid soln = solver.getSolutions().get(0);
		if(!soln.equals(TestGrid.getSolution1()))
		{
